...
MODEL_FILENAME = "99.11-DP-caformer_b36.sail_in22k_ft_in1k_384.pth"
...
CLASSIFICATION_MODEL_PATH = os.path.join(CURRENT_FILE_DIRECTORY, "models", MODEL_FILENAME)
    ...
    **Warning:** This function loads the model and creates the transform internally
    ...
    try:
        ...
        if not os.path.exists(CLASSIFICATION_MODEL_PATH):
            log.error(f"Model file not found at: {CLASSIFICATION_MODEL_PATH}")
            raise FileNotFoundError(f"Model file not found: {CLASSIFICATION_MODEL_PATH}")

        log.info(f"Loading model structure '{MODEL_NAME}'...")
        model = timm.create_model(MODEL_NAME, pretrained=False, num_classes=NUM_CLASSES)
        log.info(f"Loading state dict from: {CLASSIFICATION_MODEL_PATH}")
        checkpoint = torch.load(CLASSIFICATION_MODEL_PATH, map_location=device,weights_only=True)
