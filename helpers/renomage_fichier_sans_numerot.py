...
NOMMAGE_MODEL_NAME = "Numerotation-regnety_320.swag_ft_in1k.pth" # Use raw string
NOMMAGE_MODEL_FILENAME = "regnety_320.swag_ft_in1k"
NOMMAGE_MODEL_PATH = os.path.join(CURRENT_FILE_DIRECTORY, "./models", NOMMAGE_MODEL_FILENAME)
...
def bypass_numerot(input_folder: str, dry_run: bool = False) -> str:
    ...
    if not bypass_inference:
        ...
        try:
            model = timm.create_model(NOMMAGE_MODEL_NAME, pretrained=False, num_classes=NUM_CLASSES)
            if not os.path.exists(NOMMAGE_MODEL_PATH):
                 logging.error(f"Model weights file not found at {NOMMAGE_MODEL_PATH}")
                 return "error: model_file_not_found"
            logging.info(f"Loading weights from: {NOMMAGE_MODEL_PATH}")
            checkpoint = torch.load(NOMMAGE_MODEL_PATH, map_location='cpu') # Load to CPU first
